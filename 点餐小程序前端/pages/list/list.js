...
Page({
  ...
  onLoad: function () {
    ...
    fetch('/food/list').then(data => {
      wx.hideLoading()
      // 处理菜品图片路径，拼接完整URL
      var foodList = data.list.map(category => ({
        ...category,
        food: category.food.map(item => ({
          ...item,
      ...
      this.setData({
        ...
        promotion: data.promotion[0] ? {
          ...data.promotion[0],
          image: baseUrl + data.promotion[0].image
        } : {}
      }, () => {
  ...
  order: function() {
    ...
    // 将购物车对象转换为数组
    var cartArray = Object.values(this.data.cartList)
    console.log('购物车数据:', cartArray)
    console.log('cartList:', this.data.cartList)
    fetch('/food/order', cartArray, 'POST').then(data => {
      wx.hideLoading()
      wx.navigateTo({
